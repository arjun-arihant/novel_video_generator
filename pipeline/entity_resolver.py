...
Collects raw names from scenes_raw files, calls LLM for canonicalization,
produces scenes_canonical_ch{N}.json with consistent naming.
"""
import copy
import json
import logging
...
def _collect_raw_names(all_scenes: dict[int, dict]) -> tuple[list[str], list[str]]:
    """Walk all scenes and collect unique character and location names."""
    chars: set[str] = set()
    locs: set[str] = set()
...
def _apply_map(text: str, canon_map: dict[str, str]) -> str:
    """Apply canonical mapping to a name."""
    return canon_map.get(text, text)


def _apply_canonical_map_to_scene(
    scene: dict, char_map: dict, loc_map: dict
) -> dict:
    ...
    for item in s.get("sequence", []):
        if item.get("speaker") and item["speaker"] != "NARRATOR":
...
def resolve_entities(book_slug: str, chapter_ids: list[int]) -> None:
    ...
    Collects raw names → LLM canonicalization → saves canonical maps → writes
    scenes_canonical_ch{N}.json for every chapter.
    """
    ...
    if all_exist:
        logger.info("Stage 4 — All scenes_canonical files exist, skipping.")
        return
    ...
    if char_map_path.exists() and loc_map_path.exists():
        ...
        logger.info("Stage 4 — Loaded existing canonical maps.")
    else:
        all_scenes = _load_raw_scenes(book_slug, chapter_ids)
        raw_chars, raw_locs = _collect_raw_names(all_scenes)

        if not raw_chars and not raw_locs:
            logger.info("Stage 4 — No entity names found, using identity maps.")
            char_map = {}
            loc_map = {}
        else:
            logger.info(
                f"Stage 4 — Collected {len(raw_chars)} character names, "
                f"{len(raw_locs)} location names. Calling LLM..."
            )
        ...
        logger.info(
            f"Stage 4 — Canonical maps saved: {len(char_map)} chars, {len(loc_map)} locs."
        )
    ...
    for cid, data in all_scenes.items():
        ...
        if out_path.exists():
            logger.debug(f"Stage 4 — scenes_canonical_ch{cid}.json exists, skipping.")
            continue
        ...
        logger.info(f"Stage 4 — scenes_canonical_ch{cid}.json written.")
