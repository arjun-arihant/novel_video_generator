"""
Stage 11: Video Composer
Combines scene images + audio into chapter MP4 using FFmpeg.
Pure FFmpeg filter graphs — no moviepy dependency.
"""
import json
import logging
import shutil
...
def _concat_audio_for_scene(
    ...
    """
    Concatenate all seq_{scene_id}_*.wav files for a scene.
...
def _concat_clips_with_crossfade(clip_paths: list[str], out_path: str) -> None:
    """Concatenate scene clips with 0.3s crossfade transitions."""
    if len(clip_paths) == 1:
    ...
    for i in range(1, n):
        ...
        filter_parts.append(
            f"{stream_label}[{i}:v]xfade=transition=fade:"
...
def compose_chapter(book_slug: str, chapter_id: int) -> str:
    ...
    Returns path to output MP4, or empty string on failure.
    """
    out_mp4 = novel_path(book_slug, "output", f"chapter_{chapter_id}.mp4")
    if Path(out_mp4).exists():
        logger.info(f"Stage 11 — chapter_{chapter_id}.mp4 already exists, skipping.")
        return out_mp4
    ...
    if not Path(canonical_path).exists():
        logger.error(f"Stage 11 — No canonical scenes for ch{chapter_id}.")
        return ""
    ...
    console.print(
        f"[bold blue]▶ Stage 11 — Composing chapter {chapter_id} "
        f"({len(scenes)} scenes)[/bold blue]"
    )

    clip_paths = []

    with tempfile.TemporaryDirectory(prefix="nvg_compose_") as tmpdir:
        for scene in scenes:
            ...
            if not Path(image_path).exists():
                logger.warning(
                    f"Missing image for scene {scene_id}, ch{chapter_id}. Skipping."
                )
                continue

            scene_audio = str(Path(tmpdir) / f"scene_{scene_id:03d}_audio.wav")
            duration = _concat_audio_for_scene(
                book_slug, chapter_id, scene_id, scene_audio
            )
        ...
        if not clip_paths:
            logger.error(
                f"No valid clips for chapter {chapter_id}. Cannot compose video."
            )
            return ""
